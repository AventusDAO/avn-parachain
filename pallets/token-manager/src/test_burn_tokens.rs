    ...
    mod set_treasury_burn_threshold {
        ...
        mod succeeds_when {
            ...
            fn fund_treasury_below_threshold_does_not_move_to_burn_pot() {
                ...
                ext.execute_with(|| {
                    // TotalSupply=10_000 => threshold=1_500 (if TreasuryBurnThreshold is set at
                    // 15%)
                    ...
                    let threshold =
                        <TestRuntime as token_manager::Config>::TreasuryBurnThreshold::get() *
                            total_supply;
                    ...
                    TokenManager::fund_treasury(from.clone(), fund_amount).unwrap();
            ...
            fn fund_treasury_multiple_times_caps_treasury_and_accumulates_burn_pot() {
                ...
                ext.execute_with(|| {
                    ...
                    TokenManager::fund_treasury(from.clone(), first).unwrap();
                    assert_eq!(Curr::free_balance(&treasury), first);
                    assert_eq!(Curr::free_balance(&burn), 0u128);

                    // 2) Fund +50 => now 40 over threshold => 40 should move
                    let second = 50u128;
                    TokenManager::fund_treasury(from.clone(), second).unwrap();

                    assert_eq!(Curr::free_balance(&treasury), threshold);
                    assert_eq!(Curr::free_balance(&burn), 40u128);

                    // 3) Fund +100 => treasury already at threshold, so all 100 is excess => all
                    // moves
                    let third = 100u128;
                    TokenManager::fund_treasury(from.clone(), third).unwrap();
            ...
            fn fund_treasury_above_threshold_only_sweeps_up_to_burn_cap() {
                ...
                ext.execute_with(|| {
                    ...
                    let cap: BalanceOf<TestRuntime> =
                        <TestRuntime as token_manager::Config>::TreasuryBurnCap::get();
                    ...
                    assert_eq!(
                        Curr::free_balance(&treasury),
                        threshold.saturating_add(extra_over_cap)
                    );
                });
            }
        }
